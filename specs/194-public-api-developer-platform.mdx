...
description: API key management, per-key rate limiting, versioned public API controllers, and CLI generation for external developers and AI agents.
---
...
This spec covers the full "Developer Platform" epic (#194) which groups three dependent issues:
...
Provide a versioned, key-authenticated public API (`/api/v1/`) with per-key rate limiting, a management UI, and a build-time-generated CLI — enabling external developers and AI agents to programmatically interact with the Roxabi platform.
