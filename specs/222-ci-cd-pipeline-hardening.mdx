...
1. Developer opens a PR that only changes files in `docs/`, `analyses/`, `specs/`, root `*.md`, or `.claude/**`
...
1. CI runs the `docs-links` job using `lycheeverse/lychee-action`
2. `lychee` scans all `.mdx` and `.md` files for internal links
3. Links are checked against the filesystem (external URLs excluded via config)
...
- **External URLs:** Excluded initially via `.lychee.toml` (`exclude = ['^https?://']`) to avoid flaky CI
- **Anchor links (`#section`):** Start with `include_fragments = false` (safer default). If testing during implementation confirms reliable MDX anchor validation, enable it. Otherwise, note as a known limitation
- **Generated files:** Links to files that only exist after build (e.g., API docs) are added to the exclude list
- **MDX syntax:** JSX components and imports may confuse some checkers — lychee handles this well
...
| Link checker tool | `lychee` via `lycheeverse/lychee-action` | Fastest option, handles MDX, supports `.lychee.toml` config, transparent install |
...
- [ ] `.lychee.toml` config excludes external URLs
...
- **`lychee` anchor validation in MDX:** Does `include_fragments = true` reliably validate `#section` anchors in MDX files? Start with `false` (safer default). Test during implementation and enable as a follow-up if reliable.
