...
│  │  6. Commit plan to plans/{issue}-{slug}.mdx   │  ← NEW #282     │
...
4. Output: `analyses/280-token-consumption.mdx` + rubric added to bootstrap/triage
...
2. Spec defines new micro-task step in scaffold (between stubs and implement):
   ...
   - Commit plan to `plans/{issue}-{slug}.mdx` before agent spawning
...
| `plans/` directory doesn't exist yet | Scaffold creates `plans/` on first use. Plan file committed to worktree branch, included in PR. |
...
**Decision:** Scaffold writes its plan to `plans/{issue}-{slug}.mdx` before spawning agents. This is a durable artifact, not an ephemeral conversation-only approval.
