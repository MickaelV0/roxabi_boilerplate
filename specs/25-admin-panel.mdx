...
- Org members (`/org/members`) and org settings (`/org/settings`) exist but are basic — no pagination, no custom RBAC role support, limited to active org.
- Two-tier role system in place: global `users.role` (`user | admin | superadmin`) and org-scoped RBAC (`roles` table with `owner | admin | member | viewer` + custom roles).
- `@Roles()` and `@Permissions()` decorators/guards are implemented. `@Roles('superadmin')` is ready but unused.
- Better Auth admin plugin loaded at `/api/auth/admin/*` — bypasses NestJS guards (must be disabled/restricted in Phase 1).
- `TenantInterceptor` already supports `parentOrganizationId` on organizations (dynamic field check) — the schema column just needs to be added.
- No `/admin` routes on frontend. No admin-scoped API endpoints in NestJS.
...
### Phase 1: Admin Layout + Page Migration + Foundation

**Scope:** Admin route tree, layout, guards, migration of existing org pages, `AuditService` scaffolding, Better Auth plugin restriction, and `members.role` sync resolution.

- Create `/admin` route tree with admin layout (sidebar + content area)
- Implement `requireAdmin` and `requireSuperAdmin` route guards
- Sidebar with nav groups (Organization + System) — System group hidden for org admins
- Migrate `/org/members` → `/admin/members` with server-side pagination and full RBAC role support
- Migrate `/org/settings` → `/admin/settings`
- Add client-side redirects from old `/org/*` routes
- Backend: new `AdminModule` shell with member management endpoints (paginated list, role change, invite, remove)
- **Resolve `members.role` / `invitations.role` sync**: ensure role changes via admin API write both `roleId` and legacy `role` field (or migrate to `roleId` only if Better Auth allows)
- **Scaffold `AuditService`** with `audit_logs` table + migration. Wire into member management controllers from day one.
- **Disable/restrict Better Auth admin plugin** (`/api/auth/admin/*`) to close the unguarded endpoint security gap

### Phase 2: User &amp; Org Management

**Scope:** Cross-tenant user and organization management for super admins.

- `/admin/users`: cross-tenant user list with cursor pagination, search, filters
- User detail view: profile, org memberships, activity summary (last 10 audit log entries)
- User actions: edit details, change global role, ban/unban, soft-delete, restore
- `/admin/organizations`: cross-tenant org list with cursor pagination
- Org actions: create, edit, soft-delete (with impact preview), restore
- Org hierarchy: add `parentOrganizationId` to organizations schema (aligns with existing `TenantInterceptor`), tree view in UI, max depth 3 validation (application + DB check constraint)
- Backend: `AdminUsersController`, `AdminOrganizationsController` with `@Roles('superadmin')`
- Wire `AuditService.log()` into all new controllers

### Phase 3: System Settings + Feature Flags

**Scope:** Global configuration and feature flag management.
...
### Phase 4: Impersonation + Health Dashboard

**Scope:** User impersonation and system monitoring.
...
- [ ] **Member management**: Given an org admin session, `GET /api/admin/members` returns only members of the admin's org, paginated. Invite, role-change, and remove operations work and create audit log entries.
- [ ] **Permission-based UI**: Org admins see only the Organization nav group. Super admins see both groups. Non-admin users navigating to `/admin` are redirected to `/dashboard`. Org admins navigating to `/admin/users` are redirected to `/admin`.
- [ ] **Page migration**: Navigating to `/org/members` redirects to `/admin/members`. Navigating to `/org/settings` redirects to `/admin/settings`. All existing functionality is preserved and enhanced (pagination, all RBAC roles in invite/change dialogs).
- [ ] **Cross-tenant CRUD**: Given a super admin session, `GET /api/admin/users` returns users from all organizations. Ban, unban, soft-delete, and restore operations work and create audit log entries. Same for `GET /api/admin/organizations`.
- [ ] **Impersonation**: Super admin can start an impersonation session for a non-superadmin user. Persistent banner is visible on all pages. "Stop Impersonating" restores the original session. Attempting to impersonate a superadmin returns 403. Impersonation start and stop are audit logged.
- [ ] **Audit trail**: Every admin action listed in the action vocabulary table produces an audit log entry with correct `before`/`after` data. The audit log viewer at `/admin/audit-logs` displays entries with filters, search, and expandable diff view. Verified by integration tests.
- [ ] **Feature flags**: Super admin can create, toggle, and delete boolean feature flags. `FeatureFlagService.isEnabled(key)` returns the correct value. Duplicate key creation returns 400.
- [ ] **System settings**: Super admin can view and edit typed system settings grouped by category. Changes persist and are audit logged.
- [ ] **Health dashboard**: `/admin/health` displays API status (up/down based on health check), DB status, active users (24h), total users, total orgs, and DB pool stats. Deferred metrics show "Not configured" cards.
- [ ] **Org hierarchy**: Setting `parentOrganizationId` on an organization works. Setting a 4th-level parent returns 400. Tree view renders with correct indentation and expand/collapse. `TenantInterceptor` correctly resolves child orgs to parent tenant.
- [ ] **Soft delete**: Soft-deleted users and orgs appear when filtering by "Archived" status. Super admins can restore them via the admin panel.
- [ ] **Better Auth plugin**: `/api/auth/admin/*` endpoints are disabled or restricted. Admin actions go through NestJS controllers exclusively.

## Open Questions

- **Impersonation via Better Auth**: Does the current Better Auth version support the `impersonate-user` admin API with custom session fields? If not, impersonation session must be created via direct session manipulation. Investigate during Phase 4.
...
- **`members.role` migration path**: Phase 1 must resolve whether to write both fields on every change (safe, compatible) or migrate to `roleId` only (cleaner, may break Better Auth SDK calls). Investigate Better Auth internals before deciding.
- **Initial system settings**: What settings should exist at launch? The schema and UI are defined, but the initial settings catalog needs to be determined during Phase 3 implementation.
- **Rate limiting for admin endpoints**: The existing rate limiter has `global` (60 req/min) and `auth` (5 req/min) tiers. Consider whether admin endpoints need a separate tier or exemption for bulk operations.
