...
3. Existing Mermaid code blocks (e.g., in `analyses/00-infrastructure-setup-summary.mdx`) render as SVG diagrams instead of plain code
...
- **Existing mermaid blocks**: `analyses/00-infrastructure-setup-summary.mdx` already has a Mermaid block. Leave existing Mermaid blocks untouched.
