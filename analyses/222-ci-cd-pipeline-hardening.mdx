...
If `code` filter returns `false`, skip `lint`, `typecheck`, `test`, `build`, and `e2e` jobs. This means changes to `docs/**`, `analyses/**`, `specs/**`, root `*.md` files, and `.claude/**` naturally trigger the skip without needing explicit listing.
