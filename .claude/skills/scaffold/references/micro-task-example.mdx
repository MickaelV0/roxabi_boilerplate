...
- **Verify:** `bun run test apps/api/src/auth/jwt.guard.spec.ts` (deferred)
...
- **Verify:** `bun run test apps/api/src/users/users.service.spec.ts` (deferred)
...
- **File:** `apps/api/src/auth/jwt.guard.spec.ts`
- **Snippet:** `describe('JwtGuard', () => { it('should validate token', ...) })`
- **Verify:** `bun run test apps/api/src/auth/jwt.guard.spec.ts` (ready)
- **Expected:** Tests fail (RED phase — no implementation yet)
- **Time:** 3 min
- **Difficulty:** 2
- **Traces:** N1
- **Phase:** RED
...
- **Verify:** `bun run test apps/api/src/users/users.service.spec.ts` (ready)
- **Expected:** Tests fail (RED phase — no implementation yet)
- **Time:** 3 min
- **Difficulty:** 2
- **Traces:** N2
...
{
  "taskDifficulty": 3,
  "verificationCommand": "bun run test apps/api/src/auth/jwt.guard.spec.ts",
