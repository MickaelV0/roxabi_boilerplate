...
const oAuthButtonVariants = cva(
  'inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md border bg-background text-sm font-medium shadow-xs transition-all hover:bg-accent hover:text-accent-foreground disabled:pointer-events-none disabled:opacity-50 outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px]',
  {
    variants: {
      size: {
...
type OAuthButtonProps = React.ComponentProps<'button'> &
...
const providerIcons: Record<OAuthProvider, React.FC<{ className?: string }>> = {
  google: GoogleIcon,
  github: GitHubIcon,
}

/**
 * OAuthButton — branded button for OAuth sign-in with provider icon.
 *
 * @example
 * ```tsx
