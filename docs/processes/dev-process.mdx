...
| Artifact | When | Files |
|----------|------|-------|
| Feature spec | New feature | `specs/*.mdx` |
| Analysis (pre-spec) | Before writing a spec | `analyses/*.mdx` |
...
flowchart TD
    ...
    subgraph P1[" Phase 1 — Assessment "]
        ...
        SpecQ -- Yes --> P1End((" "))
        SpecQ -- No --> Boot["/bootstrap: interview + review"]
        Boot --> Analysis["Write analysis"]
    ...
    subgraph P2[" Phase 2 — Scaffold & Implementation "]
...
2. Main Claude fetches the GitHub issue, scans for existing documents:
   - `analyses/*` — matching analysis
   - `specs/*` — matching spec
3. **If a sufficient spec exists** — skip to Phase 2
4. **If gaps exist** (no issue detail, no analysis, no spec):
   - Main Claude runs `/bootstrap` which drives the pipeline directly:
     - Conducts interviews with the human (via `/interview`)
     - Writes the analysis, spawns expert reviewers (product-lead, doc-writer, architect if needed)
     - Human approves the analysis
     - Writes the spec, spawns expert reviewers
     - Human approves the spec
   - Once spec is approved, Main Claude proceeds to Phase 2
...
7. **Implement** — Spawns agents based on the plan (agents create files from scratch):
...
9. **PR** — `/scaffold` commits and creates the PR:
    ```bash
...
14. **Parallel domain fixers** receive the accepted findings scoped to their domain:
...
| Spec | `specs/XXX-slug.mdx` | `specs/123-user-auth.mdx` |
