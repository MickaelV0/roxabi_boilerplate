...
Deployment runs automatically on pushes to `main` via platform integrations:

- **Web** (TanStack Start) — auto-deploys to **Vercel** (free, SSR, no cold starts)
- **API** (NestJS) — auto-deploys to **Render** (free tier, cold starts after 15min idle)

No Docker or SSH needed — both platforms watch the GitHub repo directly.

> **First-time setup:** Connect your repo to Vercel and Render. See the [Deployment Guide](/docs/guides/deployment).
