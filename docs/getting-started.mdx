...
The CD pipeline runs automatically on pushes to `main` after CI passes:

1. **Build** — Docker images for API and Web are built and pushed to GHCR
2. **Deploy** — Images are pulled and started on the VPS via SSH
3. **Notify** — GitHub commit status is updated

Images are tagged `latest` and `sha-&lt;commit&gt;` for rollback capability.

> **First-time setup:** You need to configure your VPS and GitHub Secrets before deployment works. See the [Deployment Guide](/docs/guides/deployment).
