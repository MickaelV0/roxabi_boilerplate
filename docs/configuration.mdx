...
Production is split across two platforms: **Vercel** (web) and **Render** (API). Environment variables are configured in each platform's dashboard.
...
| `APP_URL` | Vercel app URL | `https://roxabi.vercel.app` |
...
| Variable | Description | Example |
|----------|-------------|---------|
| `DATABASE_URL` | PostgreSQL connection (OVH CloudDB) | `postgresql://user:pass@host:5432/roxabi` |
| `CORS_ORIGIN` | Vercel web URL | `https://roxabi.vercel.app` |
| `BETTER_AUTH_SECRET` | Auth secret (random 32+ chars) | `<random-string>` |
| `BETTER_AUTH_URL` | Render API URL | `https://roxabi-api.onrender.com` |
| `APP_URL` | Vercel web URL | `https://roxabi.vercel.app` |
| `API_URL` | Render API URL | `https://roxabi-api.onrender.com` |
| `EMAIL_FROM` | Sender email address | `noreply@example.com` |

> **Note:** Render auto-sets `PORT` (default 10000) and `NODE_ENV=production`. Do not override these.
