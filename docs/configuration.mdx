---
title: Configuration
description: Setup instructions and environment variables for Roxabi Boilerplate
---

## Prerequisites

- [Bun](https://bun.sh/) >= 1.1.0
- Node.js >= 20 (for compatibility)
- [Docker](https://www.docker.com/) (for local PostgreSQL)
- Git

## Installation

```bash
# Clone the repository
git clone <repo-url>
cd roxabi_boilerplate

# Install dependencies
bun install
```

## Environment Variables

Copy the example file and configure:

```bash
cp .env.example .env
```

### Required Variables

| Variable | Description | Example |
|----------|-------------|---------|
| `NODE_ENV` | Environment mode | `development` |
| `DATABASE_URL` | PostgreSQL connection string | `postgresql://roxabi:roxabi@localhost:5432/roxabi` |
| `JWT_SECRET` | Secret for JWT tokens | Random string |

### Optional Variables

| Variable | Description | Default |
|----------|-------------|---------|
| `APP_URL` | Frontend URL | `http://localhost:3000` |
| `API_URL` | Backend URL | `http://localhost:4000` |
| `GITHUB_TOKEN` | GitHub API access | - |

### Claude Code Agent Teams

| Variable | Description | Default |
|----------|-------------|---------|
| `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS` | Enable multi-agent coordination (experimental) | `0` |
| `CLAUDE_CODE_TEAMMATE_MODE` | Controls teammate display: `in-process` (same terminal) or `split-pane` (separate panes) | `in-process` |

To enable agent teams, set in your shell:

```bash
export CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1
```

Agent definitions are in `.claude/agents/*.md`. See the [Agent Teams Guide](/docs/guides/agent-teams) for details.

## TypeScript Configuration

Root `tsconfig.json` provides base settings. Each app/package extends it:

```json
{
  "extends": "../../tsconfig.json",
  "compilerOptions": {
    "outDir": "./dist"
  }
}
```

## Biome Configuration

Linting and formatting configured in `biome.json`:

- 2-space indentation
- Single quotes
- No semicolons (ASI)
- 100 character line width

## TurboRepo Configuration

Tasks defined in `turbo.json`:

- `dev` - Run development servers
- `build` - Build all packages
- `typecheck` - Type checking
- `lint` - Run linter
- `test` - Run tests

## IDE Setup

### VS Code

Recommended extensions:

- Biome
- TypeScript
- Turbo Console Log

Settings:

```json
{
  "editor.defaultFormatter": "biomejs.biome",
  "editor.formatOnSave": true
}
```

## Production Environment

Production uses Docker containers deployed to a VPS. Environment variables are stored in `/opt/roxabi/.env` on the VPS (not baked into Docker images).

### Production Variables

| Variable | Description | Example |
|----------|-------------|---------|
| `NODE_ENV` | Must be `production` | `production` |
| `PORT` | API server port | `4000` |
| `WEB_PORT` | Web server port | `3000` |
| `DATABASE_URL` | PostgreSQL connection (OVH CloudDB) | `postgresql://user:pass@host:5432/roxabi` |
| `CORS_ORIGIN` | Frontend URL for CORS | `https://app.example.com` |
| `BETTER_AUTH_SECRET` | Auth secret (random 32+ chars) | `<random-string>` |
| `BETTER_AUTH_URL` | Auth callback URL | `https://api.example.com` |
| `APP_URL` | Public frontend URL | `https://app.example.com` |
| `API_URL` | Public API URL | `https://api.example.com` |
| `EMAIL_FROM` | Sender email address | `noreply@example.com` |

> **Security:** The `.env` file on VPS should have `chmod 600` permissions. Never commit production secrets to the repository.

### GitHub Secrets (for CD Pipeline)

Configure these in **Settings** &gt; **Secrets and variables** &gt; **Actions**:

| Secret | Description |
|--------|-------------|
| `VPS_HOST` | VPS IP address or hostname |
| `VPS_USER` | SSH deploy user (e.g., `deploy`) |
| `VPS_SSH_KEY` | Ed25519 private key for SSH access |

These secrets are used by the CD workflow to deploy via SSH. See the [Deployment Guide](/docs/guides/deployment) for full setup instructions.
