...
- Child orgs are for organizational grouping, not security boundaries
...
The tests are automatically skipped when `DATABASE_URL` is not set, so they will not fail in CI environments without a database.
