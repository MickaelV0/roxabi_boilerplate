...
flowchart TD
    ...
    FFScaffold --> FFReview["/review + /1b1 + fixer"]
```
...
MCP tools (Context7, Vercel) are only available to the **orchestrator** (main Claude Code session). Sub-agents use `WebSearch` + `Bash` CLI equivalents instead.

| MCP Tool | Orchestrator | All Agents |
|----------|:---:|:---:|
| Context7: resolve-library-id / query-docs | ✅ | ❌ |
| Vercel: deploy, logs, get_deployment, etc. | ✅ | ❌ |

### Known Gaps

| Gap | Severity | Impact |
|-----|----------|--------|
| `TaskCreate`, `TaskGet`, `TaskUpdate`, `TaskList` missing from all agents | 🔴 Must fix | Agents cannot claim tasks, mark complete, or create follow-ups autonomously — the micro-task protocol in AGENTS.md requires these |
| `Task` missing from `fixer` | 🔴 Must fix | fixer cannot delegate to domain-specific sub-fixers for multi-domain findings |
| `WebFetch` missing from all agents | 🟡 Should add | Agents cannot fetch specific URLs (deployed pages, raw GitHub files, API responses). `WebSearch` only finds, it does not fetch a targeted URL |
| Context7 not available to agents | 🟡 Should add | doc-writer, frontend-dev, backend-dev, architect use `WebSearch` for docs — Context7 gives versioned, authoritative library documentation |
| Vercel MCP not available to `devops` | 🟡 Should add | devops uses `vercel` / `gh` CLI via Bash — Vercel MCP gives structured access to logs and deployment status |
| No skills on `doc-writer` | 🟡 Should add | Would benefit from `context7-plugin:docs` for library reference lookups while writing docs |
| No skills on `backend-dev` | 🟡 Should add | Would benefit from `context7-plugin:docs` for NestJS / Drizzle API lookups |
| No skills on `devops` | 🟡 Should add | Would benefit from `vercel:logs`, `vercel:deploy` skills |
...
| backend-dev | — | No preloaded skill |
| devops | — | No preloaded skill |
| doc-writer | — | No preloaded skill |
| fixer | — | No preloaded skill |
...
> `model: sonnet` is explicitly set in agent frontmatter for domain and quality agents. Strategy agents (product-lead, architect) inherit Opus from the orchestrator session since they are not assigned a model in frontmatter.
