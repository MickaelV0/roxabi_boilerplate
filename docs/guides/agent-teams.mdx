...
|------|---------|---------|
| `bypassPermissions` | Agent can execute all its tools freely — appropriate for domain agents that write code | frontend-dev, backend-dev, infra-ops, tester, doc-writer, product-lead, architect |
| `plan` | Agent can only read and analyze — it proposes changes but cannot execute them | reviewer, security-auditor |
...
| `40` | Product-lead (analysis + planning + verification) |
...
| doc-writer | `commit` | Conventional Commits workflow |
| reviewer | `review`, `agent-browser` | Code review + deployed verification |
| tester | `test`, `context7` | Test generation + library docs |
| architect | `plan`, `adr`, `commit`, `context7` | Planning, ADRs, commits, library docs |
| product-lead | `interview`, `issue-triage`, `issues`, `adr`, `commit`, `agent-browser`, `context7`, `1b1`, `bootstrap` | Full product ownership toolkit |
...
4. Create the PR
5. Run `/review` on the PR for structured code review
...
5. Create the PR
6. Run `/review` on the PR for structured code review
...
4. Create the PR
5. Run `/review` on the PR for a quick code review
6. Resolve any issues
...
6. Create the PR
7. Run `/review` on the PR for structured code review
