...
For the full design rationale, see the [RBAC analysis](../analyses/24-rbac).
...
| **Member** | `member` | Standard read access | Read-only on all resources + `roles:read` |
...
This pattern is used in the actual codebase. For example, in `apps/web/src/routes/org/settings.tsx`:
...
export type PermissionResource = 'users' | 'organizations' | 'members' | 'invitations' | 'roles' | 'billing'
```
...
- [RBAC analysis](../analyses/24-rbac) -- Full design rationale and architectural decisions
