...
2. Select your Vercel team/account and link to the **API project**
3. Trigger a redeploy: `vercel redeploy <latest-deployment-url>`
4. Verify: `cd apps/api && vercel env ls | grep RESEND`
...
Rate limiting is **enabled by default** (`RATE_LIMIT_ENABLED=true`) and requires Upstash Redis. The code uses `@upstash/redis` and reads `KV_REST_API_URL` and `KV_REST_API_TOKEN` — the env var names auto-injected by the Vercel Upstash integration.
...
| `KV_REST_API_URL` | Upstash Redis REST URL (for rate limiting) |
| `KV_REST_API_TOKEN` | Upstash Redis REST token (for rate limiting) |

#### Credential Rotation

| Credential | Rotation method |
|-----------|-----------------|
| `DATABASE_URL` (production) | Auto-rotates via Neon Marketplace integration — no manual action needed |
