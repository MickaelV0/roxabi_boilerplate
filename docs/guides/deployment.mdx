...
Rate limiting is **enabled by default** (`RATE_LIMIT_ENABLED=true`) and requires Upstash Redis. The code uses `@upstash/redis` and reads `UPSTASH_REDIS_REST_URL` and `UPSTASH_REDIS_REST_TOKEN`.

> **Important:** Use the **Upstash** integration, not the Vercel native Redis/KV integration. They produce different env var names. Our code expects `UPSTASH_REDIS_REST_*`, not `KV_REST_API_*`.
...
6. Vercel auto-creates `UPSTASH_REDIS_REST_URL` and `UPSTASH_REDIS_REST_TOKEN` as environment variables on the API project
...
```bash
cd apps/api && vercel env ls | grep UPSTASH
```

You should see `UPSTASH_REDIS_REST_URL` and `UPSTASH_REDIS_REST_TOKEN` listed.
...
cd apps/api
for env in production preview development; do
  printf '%s' "https://your-redis.upstash.io" | vercel env add UPSTASH_REDIS_REST_URL "$env"
  printf '%s' "your-upstash-token"             | vercel env add UPSTASH_REDIS_REST_TOKEN "$env"
done
```
...
Rate limiting is enabled by default and requires Upstash Redis. Follow the step-by-step in [Upstash Redis setup](#upstash-redis-rate-limiting) above, or set `RATE_LIMIT_ENABLED=false` temporarily if you need to deploy without it.

> **Note:** If you skip this step, the API will crash at startup in production.

### 5. Disable Automatic Preview Deployments
...
### 7. (Optional) Configure Deployment Protection
...
>   -H "Content-Type: application/json" \
>   -d '{"buildCommand":"bun run db:migrate && turbo run build","installCommand":"bun install"}'
> ```
