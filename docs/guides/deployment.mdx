...
Both projects use `ignoreCommand` in `vercel.json` to only trigger Vercel builds on the `main` branch. This is version-controlled and prevents excessive preview deployments from rate-limiting.
