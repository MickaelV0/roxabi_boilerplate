...
4. Choose a target: `web`, `api`, `docs`, or `both`
...
4. Repeat for a third project with Root Directory set to `apps/docs`
5. Link each project locally with `vercel link` (see [Initial Setup](#initial-setup) above)
...
Disable Vercel's automatic preview deployments on **all three** projects to prevent rate-limit exhaustion. See [Disable Automatic Preview Deployments](#3-disable-automatic-preview-deployments) for the curl commands.
