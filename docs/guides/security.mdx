...
The `BETTER_AUTH_SECRET` must be a random string of at least 32 characters. The API **throws on startup** if this is set to a known default (`dev-secret-do-not-use-in-production`) in any non-development environment, or on any Vercel deployment regardless of `NODE_ENV`.
