...
The frontend (port 3000) and API (port 3001) run on different ports in development. To keep cookies on the same origin (avoiding `SameSite=None`), the frontend uses a **Vite dev proxy**:
...
- Plugins must mirror the server-side plugins (organization, admin, magic link)
