...
**Configuration:** Each package has its own Vitest config extending `vitest.shared.ts` at the repo root:
...
**Threshold:** Maintain 80% coverage across lines, functions, branches, and statements.

**Ratcheting:** Enable `thresholds.autoUpdate: true` in Vitest config so the threshold automatically increases as coverage grows. It never decreases.
...
From `apps/web/src/lib/i18n/__tests__/server.test.ts`:
...
- **Frontend:** Use `vi.fn()` and `vi.spyOn()` for internal code. Use `vi.mock()` only for third-party modules that cannot be injected.
...
- **Database:** Use Testcontainers for integration tests that require a real PostgreSQL instance.
