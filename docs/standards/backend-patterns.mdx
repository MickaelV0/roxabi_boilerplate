...
Services apply the Dependency Inversion Principle not just for infrastructure like email, but for data access too. Instead of injecting `@Inject(DRIZZLE)` directly, a domain service injects a port token (e.g., `CONSENT_REPO`) whose type is a focused TypeScript interface. This decouples domain logic from ORM details, enables lightweight in-memory test adapters without a real database, and follows hexagonal architecture principles where the domain core depends only on abstractions it defines.

**Port interface** — defines the contract, lives in the feature module:
...
**Key rules for port design:**

- Method names mirror actual service queries, not generic CRUD (`getLatestByUserId`, not `findOne`)
- No speculative methods — only what the service currently calls
- All methods accept `tx?: DrizzleTx` for optional transaction pass-through

**Drizzle adapter** — implements the port, lives in `repositories/`:

```typescript
// repositories/drizzleConsent.repository.ts
@Injectable()
export class DrizzleConsentRepository implements ConsentRepository {
  constructor(@Inject(DRIZZLE) private readonly db: DrizzleDB) {}
  // ... query logic moved from service
}
```

**Module binding** — composition root wires the abstraction to the implementation:
...
- `*.repository.ts` — port interfaces (e.g., `consent.repository.ts`)
- `repositories/drizzle*.repository.ts` — Drizzle adapters (e.g., `repositories/drizzleConsent.repository.ts`)
- `repositories/inMemory*.repository.ts` — in-memory test adapters (e.g., `repositories/inMemoryConsent.repository.ts`)

**Rule:** `@Inject(DRIZZLE)` belongs only in Drizzle adapter files inside `repositories/`. Services must never inject `DRIZZLE` directly.
...
**Repository abstraction:** The repository pattern extends DIP to data access. Services inject a port token (e.g., `CONSENT_REPO`) rather than `DRIZZLE` directly, keeping ORM details out of domain logic. See [§1.2.1 Repository Pattern](#121-repository-pattern) for the full pattern, file conventions, and Drizzle adapter structure.
