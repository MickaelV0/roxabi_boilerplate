...
|---------|--------|---------|
| Hook Composition | Paraglide runtime (`apps/web/src/paraglide/`) | `getLocale`, `setLocale` -- focused, single-purpose utilities |
...
| Component Composition | `apps/web/src/components/LocaleSwitcher.tsx` | Variant props + companion hook for flexible rendering |
...
- **Extract logic into companion hooks** when a component's logic grows beyond simple event handlers. See `useLocaleSwitcher` in `LocaleSwitcher.tsx`.
