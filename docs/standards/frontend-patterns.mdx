...
**Before — inline definition (avoid):**

```tsx
// Scattered across multiple files; key is not reusable
...
**After — `queryOptions()` factory (required):**
...
- Wrap `queryKey` + `queryFn` in a `queryOptions()` factory — never define them inline in
  `useQuery` or `useSuspenseQuery`
- Define query keys in a central key factory (`queryKeys.ts`) — never hardcode key arrays at
  call sites
...
React has no class inheritance to substitute. Component contracts are enforced by TypeScript prop types: any component satisfying a prop interface is a valid substitute. No code example needed — TypeScript's structural typing provides this guarantee automatically.
...
- Always use `queryOptions()` factories -- never inline query config in `useQuery()`
- Centralize keys in a `queryKeys` factory per domain -- never hardcode string arrays
