...
**Continuous Deployment via Vercel + Render**
- Merge to main = auto-deploy to both platforms
- Web (TanStack Start) → Vercel (free, SSR, edge caching)
- API (NestJS) → Render (free tier, Node.js)
- Docker configs included for self-hosted/VPS alternative

### Hotfix Process

- Direct commit to `main` with post-deploy validation
- Rollback via Vercel (promote previous deployment) or Render (redeploy previous commit)
