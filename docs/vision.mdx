...
**Continuous Deployment to VPS**
- Merge to main = Docker images built → pushed to GHCR → deployed to VPS via SSH
- Docker Compose orchestrates API and Web containers
- Nginx reverse proxy with SSL (Certbot)
- Rollback by pinning a previous image SHA tag

### Hotfix Process

- Direct commit to `main` with post-deploy validation
- Rollback: pin previous `sha-&lt;commit&gt;` tag in `docker-compose.prod.yml`
