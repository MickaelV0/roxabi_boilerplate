...
The `tests/` directory mirrors `src/` structure with separate `stubs/` and `factories/` subdirectories — a disciplined test layout that roxabi lacks. The `docs/specs/` directory with formal SPEC documents is a strong parallel to roxabi's `docs/specs/` workflow.
...
`docs/specs/` contains formal SPEC-NNN documents (e.g., SPEC-003). roxabi also has `docs/specs/` but the SPEC numbering convention and formal structure are stronger in review-flow.
