...
|------|--------|-----------|
| What it does | 🟡 | Personal AI assistant — different problem domain, but deeply adjacent |
| How it works | 🟢 | Multi-channel gateway + skills + sub-agent orchestration at production scale |
| Architecture | 🟢 | Flat-src monorepo with native apps — far larger scope, battle-tested patterns |
| File structure | 🟢 | Per-feature colocation with `.test.ts` sidecars, granular module splitting |
| Tech stack | 🟢 | Oxlint + Oxfmt instead of Biome — faster, leaner linting story |
| DX | 🟢 | Per-platform dev scripts, docs-only CI skip, prek hooks, dual pnpm+bun |
| Testing | 🟢 | 70% coverage gate, unit/e2e/live/docker tiers, slowest-test reporting |
| CI/CD | 🟢 | Scope-aware CI (docs-only skip), multi-OS matrix, secrets scanning baseline |
| Documentation | 🟢 | Mintlify docs site, zh-CN i18n pipeline, AGENTS.md + CLAUDE.md symlink |
| Unique ideas | 🟢 | Skills marketplace, QMD memory, prek pre-commit tool, detect-secrets baseline |

## Detailed Analysis

### 1. What It Does 🟡

OpenClaw is an installable personal AI assistant CLI (npm package `openclaw`) that routes messages from any messaging platform (WhatsApp, Telegram, Slack, Discord, iMessage, Signal, Teams, Matrix, Zalo, WebChat) to a local AI agent runtime. It is not a SaaS boilerplate — it is a shipped product with 200k+ stars and 36k+ forks.

- **Purpose:** Own-your-data personal AI — run Claude/OpenAI/Gemini from your own devices across all messaging apps
...
Compared to roxabi (which is a SaaS web application boilerplate), openclaw is a CLI daemon product. The domains are different, but the engineering problems — TypeScript monorepo at scale, testing discipline, AI integration, CI/CD — overlap significantly.
...
4. **Skills** are installable npm packages that extend agent tool capabilities — they are loaded at runtime from `~/.openclaw/skills/`
...
```
src/           # Core TypeScript source — all modules colocated here
...
Key design patterns:
- **Protocol-first:** A JSON schema (`dist/protocol.schema.json`) is code-generated from TypeScript and used to generate Swift models for native apps — `pnpm protocol:check` enforces no drift
- **Plugin SDK:** Published as `openclaw/plugin-sdk` — skills depend on it as a peer dep, avoiding workspace coupling
...
- `.shellcheckrc` — Shell script lint
- `.markdownlint-cli2.jsonc` — Markdown lint
- `.swiftlint.yml` / `.swiftformat` — Swift tooling
- `.pre-commit-config.yaml` — Pre-commit hook config (uses `prek`)
- `.secrets.baseline` / `.detect-secrets.cfg` — Secret scanning baseline
...
| Pre-commit scanning | **detect-secrets** | — |
...
- **Dual runtime support:** `pnpm install` for production/CI, `bun install` for dev speed — both lockfiles kept in sync
- **Docs-only CI skip:** CI detects docs-only changes and skips expensive jobs (test, build, Windows, macOS, Android) — only lint and format always run
- **Scope-aware CI:** Changed files are analyzed to decide whether to run Node, macOS Swift, or Android Gradle lanes — avoids running all CI on every PR
- **Per-platform dev scripts:** `pnpm ios:run`, `pnpm android:run`, `pnpm gateway:dev`, `pnpm mac:open` — each platform has dedicated dev entry points
- **`openclaw doctor`** CLI command for config/service diagnostics
- **Interactive onboarding wizard** (`openclaw onboard --install-daemon`) — zero-friction first run
- **prek** hooks: run same checks as CI (lint + format + type)
- **3 release channels:** `stable` (tagged), `beta` (prerelease), `dev` (main HEAD)
- **LOC guardrail:** Enforced by `scripts/check-ts-max-loc.ts --max 500` — files over ~500 LOC trigger a warning
...
- **Multiple vitest configs:**
  - `vitest.unit.config.ts` — fast unit tests only (excludes gateway and extensions)
  - `vitest.config.ts` — full unit suite
  - `vitest.e2e.config.ts` — e2e tests (real filesystem/process)
  - `vitest.live.config.ts` — live integration tests (real API keys)
  - `vitest.extensions.config.ts` — extension-scoped tests
  - `vitest.gateway.config.ts` — gateway-scoped tests
- **Test tiers:**
  - Unit: `*.test.ts`
...
- **Slowest-test reporting:** `scripts/vitest-slowest.mjs` generates a Markdown report of top-50 slowest tests — uploaded as CI artifact
...
**Scope detection:**
```yaml
docs-scope:      # detects docs-only changes → skips heavy jobs
changed-scope:   # detects which platforms changed (Node/macOS/Android)
```

**Job graph:**
- `check` — types + lint + oxfmt (always runs except docs-only)
- `check-docs` — markdown lint + broken links (only when docs changed)
- `secrets` — `detect-secrets` against `.secrets.baseline`
- `build-artifacts` — builds `dist/` once, uploads as artifact for downstream jobs
- `checks` — vitest (Node + Bun matrix) + protocol check
- `checks-windows` — lint + test + protocol on Windows
- `release-check` — validates npm pack contents (push to main only)
- `macos` — Swift lint + build + test (consolidated single runner to conserve macOS quota)
- `android` — Gradle test + build matrix

**CI provider:** Blacksmith (custom fast GitHub Actions runners), not standard `ubuntu-latest`

**Secret scanning:** `detect-secrets` with a `.secrets.baseline` committed to the repo — incremental scanning against the baseline rather than full history scan on every PR.
...
- **Mintlify docs site** at `docs.openclaw.ai` — structured, searchable, versioned MDX docs
- **AGENTS.md** (AI coding guidelines) is extremely comprehensive — covers module organization, commit conventions, test guidelines, security, release flow, multi-agent safety rules, and shorthand commands
- **`CLAUDE.md` is a symlink** to `AGENTS.md` — single source of truth across AI tools
- **zh-CN i18n pipeline** — English docs are the source of truth; a glossary + translation memory pipeline auto-generates Chinese docs under `docs/zh-CN/`
- **Docs link auditing** — `scripts/docs-link-audit.mjs` + `check-docs` CI step validate internal links
- **Changelog convention** — user-facing changes only, no internal/meta notes
- **Release docs** — dedicated `docs/reference/RELEASING.md` and `docs/platforms/mac/release.md`

**Comparison to roxabi:** roxabi has strong MDX docs co-located in the app. OpenClaw's AGENTS.md depth (multi-agent safety rules, shorthand commands, per-tool notes) is significantly more detailed. The `CLAUDE.md → AGENTS.md` symlink pattern is worth adopting. Docs i18n is out of scope for roxabi currently.

### 10. Unique / Novel Ideas 🟢

Several patterns stand out as genuinely novel:

1. **Skills marketplace (ClawHub):** Skills are npm packages installed at runtime. Each skill is a directory with a `tools.md` manifest. The agent runtime dynamically loads skills and injects their tools into the context. This is a plugin architecture for AI capabilities — entirely different from roxabi's static extension model.
...
9. **`openclaw doctor`** — a diagnostic CLI command that checks config, service health, and common issues. The AI assistant can run this autonomously when users report problems.
...
Adoption: roxabi already uses CLAUDE.md well — the symlink pattern is useful only if multiple AI tools are used simultaneously.
...
**1. Multi-provider AI:** OpenClaw supports Anthropic, OpenAI, Gemini, Ollama, HuggingFace, Cloudflare AI Gateway, Bedrock, and more — with failover and auth profile rotation. 2ndBrain uses a fixed Claude integration.
...
1. **Scope-aware CI with docs-only skip** — Reduces CI costs and merge queue latency as roxabi docs grow. Already has the `dorny/paths-filter` infrastructure in CI. File: `.github/workflows/ci.yml`. Estimated effort: 1-2 days.

2. **Vitest coverage threshold (70%)** — Prevents coverage regression silently. Add to `vitest.config.ts`: `coverage: { thresholds: { lines: 70, branches: 70, functions: 70, statements: 70 } }`. Estimated effort: 1h + fixing failing threshold.

3. **detect-secrets baseline in CI** — Addresses a real security gap. Create `.secrets.baseline`, add `secrets` job to CI. Estimated effort: half day.

**Nice-to-have:**

4. **Test tier separation (unit vs integration)** — Create `vitest.unit.config.ts` excluding DB-dependent tests. Run fast unit suite on every commit, full suite only on PR + push. Estimated effort: 1-2 days.

5. **Slowest-test CI artifact** — Low-effort visibility into test performance regression. Port `scripts/vitest-slowest.mjs` pattern. Estimated effort: half day.

6. **Docs link audit script** — Catches dead links in the docs MDX. Estimated effort: 1 day.

**Potential issues to create:**

- `feat: add scope-aware CI docs-only skip` → linked to epic #163
- `feat: enforce Vitest coverage thresholds (70%)` → linked to epic #163
- `chore: add detect-secrets baseline and CI secret scanning` → linked to epic #163
- `chore: split vitest unit vs integration configs` → linked to epic #163
...
The **detect-secrets baseline** is a one-time setup that addresses a real gap — roxabi has no secret scanning today. It pairs well with the existing `.env.example` discipline.
...
The **QMD memory architecture** (`src/memory/`) is high-value inspiration if roxabi ever adds AI memory features — hybrid search, temporal decay, and embedding batching are production-proven patterns available to study.
