...
# Staging Environment with On-Demand Preview Deploys
...
This builds on the branching strategy established in [Issue #13 analysis](/docs/analyses/13-workflow-branching) (now deprecated), which recommended GitHub Flow. Issue #114 evolves this by adding a `staging` integration branch while keeping `main` as the production-only branch.
...
- Create spec for implementation details → `docs/specs/114-staging-environment-preview-deploys.mdx`
