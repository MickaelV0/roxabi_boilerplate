...
# Agent Teams Architecture for Roxabi Boilerplate
...
| **infra-ops** | CI/CD, deps, config | sonnet | Read, Write, Edit, Glob, Grep, Bash | `packages/config`, root configs |

**Rationale**: Domain agents need write access (they implement code). Opus for frontend/backend because implementation requires deep reasoning. Sonnet for infra-ops because config tasks are more mechanical.
...
- infra-ops reads `docs/configuration.mdx`
