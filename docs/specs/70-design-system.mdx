...
# #70 — Design System Page & Theme Customization
...
| Window resize / mobile viewport | Theme editor sidebar collapses to an overlay or bottom sheet on narrow viewports. Component showcase adapts responsively. |
...
│   └── minimal.ts
└── lib/
    └── theme.ts               # ThemeConfig type + apply/reset/load helpers
```
...
- [ ] Wave 2 components (8) are showcased with default rendering (interactive controls deferred)
- [ ] At least 3 composition patterns are displayed (auth forms, feedback, data display)
...
|----------|----------|-----------|
| Code snippet library | **shiki** | Already a dependency via Fumadocs. No new bundle cost. |
