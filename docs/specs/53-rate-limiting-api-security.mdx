...
1. API starts with `trustProxy: true` on the Fastify adapter. `ThrottlerModule` registers with Upstash Redis storage and two active throttler configs (global, auth). Public API tier is configured but not registered until API keys are implemented.
  ...
  useFactory: (config: ConfigService) => ({
    ...
    storage: upstashConfigured
      ? new UpstashThrottlerStorage(
          config.get('UPSTASH_REDIS_REST_URL'),
          config.get('UPSTASH_REDIS_REST_TOKEN'),
        )
...
The FastifyAdapter must be created with `trustProxy: true`:
...
Preview deployments use in-memory rate limiting (no Upstash) since `UPSTASH_REDIS_REST_URL` is not set. Swagger is enabled in previews for testing.
...
|----------|-------|--------|
| `UPSTASH_REDIS_REST_URL` | Auto-injected | Vercel Marketplace &rarr; Upstash Redis |
| `UPSTASH_REDIS_REST_TOKEN` | Auto-injected | Vercel Marketplace &rarr; Upstash Redis |
...
| `apps/api/src/index.ts` | Modify | Add `trustProxy: true` to FastifyAdapter, add `onSend` hook for rate limit headers, gate Swagger UI behind `SWAGGER_ENABLED` |
...
| `apps/api/turbo.json` | Modify | Add to `passThroughEnv`: `UPSTASH_REDIS_REST_URL`, `UPSTASH_REDIS_REST_TOKEN`, `RATE_LIMIT_ENABLED`, `SWAGGER_ENABLED`, `RATE_LIMIT_GLOBAL_TTL`, `RATE_LIMIT_GLOBAL_LIMIT`, `RATE_LIMIT_AUTH_TTL`, `RATE_LIMIT_AUTH_LIMIT`, `RATE_LIMIT_AUTH_BLOCK_DURATION`, `RATE_LIMIT_API_TTL`, `RATE_LIMIT_API_LIMIT` |
...
|----------|---------|----------|-------------|
| `UPSTASH_REDIS_REST_URL` | — | Production only | Upstash Redis REST URL (Vercel Marketplace auto-injects this name) |
| `UPSTASH_REDIS_REST_TOKEN` | — | Production only | Upstash Redis REST token (Vercel Marketplace auto-injects this name) |
...
**Env validation strategy:** `UPSTASH_REDIS_REST_URL` and `UPSTASH_REDIS_REST_TOKEN` are validated as optional in development (falls back to in-memory with a startup warning) and required in production, following the same conditional pattern used for `BETTER_AUTH_SECRET`.
...
- [ ] `trustProxy: true` configured on the FastifyAdapter
...
- **Upstash provisioning**: Should Upstash be provisioned via Vercel Marketplace integration or standalone? (Can be decided during implementation — both work identically from the code side. Marketplace auto-injects `UPSTASH_REDIS_REST_URL` and `UPSTASH_REDIS_REST_TOKEN`.)
