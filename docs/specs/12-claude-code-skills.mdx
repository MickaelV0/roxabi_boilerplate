...
```
/plan --spec 42               → Generate plan from spec docs/specs/42-*.mdx
...
1. **Read the spec:**
   - Locate spec by issue number (`docs/specs/{N}-*.mdx`) or by path
...
- **Skill independence:** Each skill must work standalone (except `/bootstrap` which calls `/interview` + `/plan`, and `/scaffold` which calls `/commit` + `/pr`)
