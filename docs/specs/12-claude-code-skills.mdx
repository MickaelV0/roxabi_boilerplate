...
description: Specification for 8 Claude Code skills covering the full dev lifecycle — from idea to merged PR.
---
...
**Reference:** [Usage Insights Analysis](/docs/analyses/claude-code-usage-insights)

**Blocks:** Every future feature implementation benefits from these skills.

## Goal

Deliver 8 skills across 3 phases that cover the **full development lifecycle**:

```
Idea → Analysis → Spec → Plan → Scaffold → Code → Test → Review → Commit → PR
 ↑                                 ↑                          ↑           ↑
 /interview (improve)           /scaffold                   /review     /commit
 /bootstrap ────────────────────┘                           /test       /pr
                                                                        /adr
```
...
| Plan approval | `/bootstrap` | Implementation plan before scaffolding |
...
Skills that call or suggest other skills:

| Caller | Calls / Suggests | Relationship |
|--------|-----------------|--------------|
| `/bootstrap` | → `/interview` | Calls for analysis and spec generation |
| `/scaffold` | → `/commit` | Calls to commit scaffold |
| `/scaffold` | → `/pr` | Calls to create draft PR |
| `/review --fix` | → `/commit` | Suggests after user approves fixes |

All other skills are fully standalone.
...
| Skill | Depends on |
|-------|------------|
| `/bootstrap` | `/interview` (calls it for analysis/spec steps) |
...
| Type | Output | Purpose |
|------|--------|---------|
| **Brainstorm** | `docs/analyses/{slug}.mdx` (with `type: brainstorm` in frontmatter) | Explore ideas, divergent thinking, no structure imposed |
| **Analysis** | `docs/analyses/{slug}.mdx` | Structured investigation of a topic |
| **Spec** | `docs/specs/{slug}.mdx` | Technical specification for implementation |
...
```
/interview --promote docs/analyses/12-skills.mdx
```
...
- `docs/analyses/` for existing analyses matching the topic
- `docs/specs/` for existing specs
...
4. **Spawn fix agents:**
   - **Parallel across different files**, sequential within the same file (bugs fixed first, then style)
   ...
   - If typecheck or tests break: **revert that fix** and mark as "could not auto-fix"

5. **Present fix report:**
...
- [ ] `--fix` spawns fix agents (parallel across files, sequential within same file)
- [ ] Fix agents revert on typecheck/test failure
...
Planning orchestrator from idea to approved implementation plan. Multiple entry points depending on what already exists. Calls `/interview` for analysis/spec steps. Three validation gates. Stops at the approved plan — execution is handled by `/scaffold`.
...
│    - docs/analyses/{issue-slug}.mdx                          │
│    - docs/specs/{issue-slug}.mdx                             │
│    - GitHub issue body                                       │
│                                                              │
│ 2. If no analysis exists:                                    │
│    - Call /interview in Analysis mode                        │
│    - Produces docs/analyses/{slug}.mdx                       │
│                                                              │
...
│    - Produces docs/specs/{slug}.mdx                          │
│                                                              │
│ 5. Present spec → User approves ✓                            │
├──────────────────────────────────────────────────────────────┤
│ GATE 3: Implementation Plan                                  │
│                                                              │
│ 6. Break spec into tasks:                                    │
│    - Identify files to create/modify                         │
│    - Auto-suggest tier (S/M/L), user confirms or overrides   │
...
| `--spec 42` | Gate 2 (Spec) | Analysis (assumes spec exists in `docs/specs/42-*.mdx`) |
...
- **Skill independence:** Each skill must work standalone (except `/bootstrap` which calls `/interview`, and `/scaffold` which calls `/commit` + `/pr`)
...
- [ ] All 8 skills implemented and documented
- [ ] Phase 1 skills (`/interview`, `/commit`, `/pr`) working independently
- [ ] Phase 2 skills (`/review`, `/test`) working with `--fix` and `--e2e` flags
- [ ] Phase 3 skills (`/bootstrap`, `/scaffold`, `/adr`) working with all entry points
- [ ] `/bootstrap` successfully calls `/interview` for analysis/spec steps
- [ ] `/bootstrap` stops at approved plan (does not scaffold)
- [ ] `/scaffold` creates worktree, scaffolds, commits, and opens draft PR
- [ ] `/review --fix` spawns fix agents (parallel across files, sequential within) without auto-committing
- [ ] CLAUDE.md updated with all new skills
- [ ] Existing skills unmodified

## Resolved Decisions

1. **`/review --fix` agent isolation:** Fix agents run in parallel across different files, sequentially within the same file (bugs first, then style). No file-level conflicts.
