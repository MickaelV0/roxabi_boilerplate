...
- **False-positive rate validation**: Before shipping, run the signal detection against 2-3 real analysis documents from `docs/analyses/` to verify the heuristic doesn't over-trigger. If most analyses fire the prompt, the detection threshold needs tightening.
