...
pre-push:
  ...
  commands:
    lint:
      # Biome runs at root level (fast ~6ms), no need for affected filter
      run: bun run lint
    typecheck:
...
> **Note:** Lint runs without `--filter` because Biome operates at root level and is extremely fast (~6ms for all files). TypeScript and tests use the affected filter for performance.
