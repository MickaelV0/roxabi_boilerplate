...
flowchart TD
    ...
    CHECK2 -- "session found" --> CHECK3{"3. @Roles check\nuser.role in list?"}
    CHECK3 -- No --> DENY2["403 Forbidden"]
    CHECK3 -- Yes --> CHECK4{"4. @RequireOrg check\nactiveOrganizationId?"}
...
```mermaid
flowchart TD
    REQ["Client request\n(with session cookie/token)"] --> GUARD["AuthGuard.canActivate()"]
    GUARD --> AUTH["AuthService.getSession(request)"]
    AUTH --> BA["Better Auth:\nvalidate token, return user + session"]
    AUTH --> PERM["PermissionService.getPermissions(userId, orgId)"]
    PERM --> Q1["Query members table:\nfind roleId for (userId, orgId)"]
    PERM --> Q2["Query role_permissions JOIN permissions:\nget resource:action strings"]
    Q1 --> RET["Return string[]\ne.g., users:read, members:write"]
    Q2 --> RET
