...
| `AdminMembersService` | `apps/api/src/admin/admin-members.service.ts` | Admin operations require organization-scoped access derived from the session's `activeOrganizationId`, not from RLS policies | Explicit `WHERE` on `organizationId` in every query |
