...
4. **Tenant isolation** — Downstream services use it to set the PostgreSQL `app.current_tenant` variable for RLS
...
| `*` | **Yes** | **Reject** — returns `false`, logs warning |
...
The CSP starts restrictive (`defaultSrc: 'none'`) and opens specific directives. The `unpkg.com` allowlist is for Swagger UI assets in non-production environments.
