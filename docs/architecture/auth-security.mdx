...
```mermaid
flowchart TD
    CLIENT["Client Browser\n(Better Auth SDK)"] -- "Cookie-based sessions" --> APILAYER
    ...
    subgraph APILAYER["NestJS + Fastify API"]
        ...
        subgraph AUTHSVC["AuthService"]
            direction LR
            BA["Better Auth Instance\n(auth.instance)"]
            PERM["Permission Service\n(RBAC)"]
        end

        BA -- "Drizzle adapter" --> PG["PostgreSQL\n(auth tables)"]
    end
```
