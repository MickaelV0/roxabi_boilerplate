...
Production deployment uses Docker containers on a VPS, with GitHub Actions for CI/CD.

```txt
┌──────────────┐     ┌──────────────┐     ┌──────────────┐
│   Developer  │────▶│   GitHub     │────▶│    GHCR      │
│  Push/Merge  │     │   Actions    │     │  (Registry)  │
└──────────────┘     └──────────────┘     └──────┬───────┘
                                                  │ docker pull
                                                  ▼
┌──────────────────────────────────────────────────────────┐
...
│                                                            │
│  ┌─────────┐    ┌───────────────┐    ┌───────────────┐    │
│  │  Nginx  │───▶│  roxabi-web   │    │  roxabi-api   │    │
│  │ :80/443 │    │  :3000        │    │  :4000        │    │
│  │         │───▶│  (Nitro SSR)  │───▶│  (NestJS)     │    │
│  └─────────┘    └───────────────┘    └───────┬───────┘    │
│                                               │            │
│  ┌─────────────┐                              │            │
│  │  WordPress  │  (existing, untouched)       │            │
│  └─────────────┘                              │            │
└───────────────────────────────────────────────┼────────────┘
                                                │
                                                ▼
...
Key decisions:
- **Docker images built in CI** — VPS has limited resources (4 GB RAM)
- **Ports bound to 127.0.0.1** — Nginx handles external traffic and SSL
- **No database container** — PostgreSQL managed by OVH CloudDB
- **WordPress coexistence** — Fully containerized, no port or filesystem conflicts
