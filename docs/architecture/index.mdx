...
Production uses **Vercel** (web) and **Render** (API) with automatic deploys from GitHub.
...
Key decisions:
- **Platform-managed deployment** — No servers to maintain, automatic SSL
- **Vercel for SSR** — TanStack Start + Nitro has native Vercel support
- **Render for API** — Free tier with Node.js, auto-deploy from GitHub
- **No database hosting** — PostgreSQL managed by OVH CloudDB
