...
├── analyses/                 # Pre-spec analyses & research (not served by Fumadocs)
├── specs/                    # Feature specifications (not served by Fumadocs)
...
- **`env`** (root `turbo.jsonc`) — Variables that affect build output (`API_URL`, `APP_URL`, `VITE_ENABLE_DEMO`). Changing these invalidates the Turbo cache.
