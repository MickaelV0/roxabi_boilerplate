...
**Cross-module event consumer.** `apps/api/src/rbac/rbac.listener.ts` (`RbacListener`) subscribes to `ORGANIZATION_CREATED` and reacts by seeding default roles. This is the bounded-context integration pattern: the RBAC module reacts to events from the organization domain without being directly imported by it.
...
The `PermissionString` type in `packages/types/src/rbac.ts` is already a branded-type pattern — a template literal union that prevents typos like `member:write`. Extend this approach to entity identifiers.
...
The motivation for introducing a shared kernel is already visible in the codebase. `apps/api/src/admin/exceptions/orgNotFound.exception.ts` and `apps/api/src/organization/exceptions/orgNotFound.exception.ts` are semantically identical — both represent the same domain concept ("an organization with this ID does not exist") but are defined independently. This duplicates domain knowledge: if the error message or constructor signature ever needs to change, both files must be updated separately, and consumers in different modules import different classes that mean the same thing.
