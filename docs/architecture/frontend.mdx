...
flowchart TD
    ...
    B --> C["Paraglide middleware\n(locale detection / URL rewriting)"]
    C --> D["TanStack Start server entry"]
    D --> E["Route matching\n(file-based route tree)"]
    E --> F["beforeLoad guards\n(auth checks, feature flags)"]
    F --> G["loader() execution\n(data fetching, server functions)"]
...
```mermaid
flowchart TD
    A["docs/\n(repo root, MDX files)"] --> B["apps/web/docs\n(symlink to ../../docs)"]
    B --> C["source.config.ts\n(Fumadocs MDX config, async loading)"]
    C --> D["fumadocs-mdx/vite\n(Vite plugin, compiles MDX at build)"]
    D --> E["Virtual modules"]
    E --> E1["fumadocs-mdx:collections/server\n(page tree, frontmatter)"]
    E --> E2["fumadocs-mdx:collections/browser\n(MDX components, TOC)"]
    E1 --> F["routes/docs/$.tsx\n(catch-all route, renders DocsLayout)"]
    E2 --> F
```
