...
Browser Request
  ...
  Nitro (server.ts)
     |
     |--> Paraglide middleware (locale detection / URL rewriting)
     |
     v
  TanStack Start server entry
     |
     |--> Route matching (file-based route tree)
     |--> beforeLoad guards (auth checks, feature flags)
     |--> loader() execution (data fetching, server functions)
     |--> React SSR render
     |
     v
  ...
  Client Hydration
     ...
     |--> TanStack Query rehydrates server-fetched data
```
...
Nitro proxies `/api/**` requests to the NestJS backend. The target is read from the `API_URL` environment variable (defaulting to `http://localhost:3001`):
...
```ts
// routes/demo.tsx -- layout wrapping /demo/* children
...
```txt
docs/              (repo root, MDX files)
  |
  v
apps/web/docs      (symlink to ../../docs)
  |
  v
source.config.ts   (Fumadocs MDX config, async loading)
  |
  v
fumadocs-mdx/vite  (Vite plugin, compiles MDX at build)
  |
  v
Virtual modules:
  fumadocs-mdx:collections/server  (page tree, frontmatter)
  fumadocs-mdx:collections/browser (MDX components, TOC)
  |
  v
routes/docs/$.tsx   (catch-all route, renders DocsLayout)
```
...
```txt
Route loader (server) --> serialized into HTML --> Route.useLoaderData() (client)
```

**SSR with TanStack Query** -- Server-fetched queries are dehydrated and rehydrated:

```txt
Query prefetch (server) --> dehydrated state --> QueryClient rehydrate (client)
```

**Client-side API call** -- Post-hydration data fetching through the Nitro proxy:

```txt
Component --> fetch('/api/...') --> Nitro proxy --> NestJS API --> Response
```

**Server function RPC** -- Type-safe server calls from client components:

```txt
Component --> createServerFn call --> Nitro RPC --> handler (server) --> Response
```
...
| TanStack Store Devtools | Store state inspection |
