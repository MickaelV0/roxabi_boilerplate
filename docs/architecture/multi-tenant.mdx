...
> **Planned / not yet implemented:** Full organization hierarchy resolution is pending Phase 3. The `resolveParentOrg()` function currently returns `orgId` directly (no-op stub). The description below reflects the intended design.
