---
issue: 388
title: "docs: documentation accuracy audit — fix outdated and missing content"
tier: F-lite
status: draft
date: 2026-02-26
...
devops, architect). The audit identified 30+ specific inaccuracies across 20+ doc files — ranging
...
All documentation accurately reflects the current codebase — every code snippet, command, file
...
migration debt explicitly noted — distinguishing current state from intended target.

## Breadboard

N/A — documentation fixes only. No UI affordances or API endpoints to wire.
...
|---|-------|-------|----------|
| 1 | **Backend docs** | `docs/standards/backend-patterns.mdx`, `docs/architecture/auth-security.mdx`, `docs/architecture/rbac.mdx`, `docs/architecture/multi-tenant.mdx`, `docs/guides/rbac.mdx`, `docs/architecture/database.mdx`, `docs/architecture/ubiquitous-language.mdx` | 🔴 High |
| 2 | **DevOps docs** | `docs/architecture/ci-cd.mdx`, `docs/hooks.mdx`, `docs/guides/i18n.mdx`, `docs/configuration.mdx`, `docs/guides/deployment.mdx`, `docs/getting-started.mdx`, `docs/contributing.mdx`, `docs/guides/security.mdx` | 🔴 High |
| 3 | **Architecture & process docs** | `docs/guides/agent-teams.mdx`, `docs/architecture/index.mdx`, `docs/product/vision.mdx`, `docs/product/persona-gtm.mdx`, `AGENTS.md`, `docs/processes/dev-process.mdx` | 🔴 High |
| 4 | **Frontend docs** | `docs/standards/frontend-patterns.mdx`, `docs/architecture/frontend.mdx`, `docs/guides/authentication.mdx`, `docs/guides/navigation-guards.mdx`, `apps/web/CLAUDE.md` | 🔴 High |
...
### Architecture / Process — High priority

- [ ] `agent-teams.mdx`: Model Tiering section updated — states that 6 agents explicitly set `model: sonnet` (backend-dev, devops, doc-writer, fixer, frontend-dev, tester) and 3 inherit (architect, product-lead, security-auditor); removes or corrects "not applied yet" claim
- [ ] `agent-teams.mdx`: broken Mermaid diagram fixed — `FFScaffold` → `FFAgents` on the dangling reference line
...
### Architecture / Process — Medium priority
...
- [ ] `AGENTS.md`: maxTurns range corrected to "30–50" (verify against `.claude/agents/*.md` frontmatter — actual values are 30 and 50)
- [ ] `docs/processes/dev-process.mdx`: numbering gap fixed in Tier F steps section (step 10 is missing)

### Frontend — High priority

- [ ] `frontend-patterns.mdx` §1.7: `queryOptions` factory pattern framed as **Target Pattern** (required standard, migration in progress); current inline `useQuery` + standalone constant shown as **Current Pattern**; prescriptive rules in summary table (e.g., "Always: wrap in `queryOptions()` factory") updated to read as "Target: wrap in `queryOptions()` factory" to avoid internal contradiction
- [ ] `frontend-patterns.mdx` §1.7: non-existent factory files (`apps/web/src/lib/featureFlags/queries.ts`, `queryKeys.ts`) removed from examples; examples updated to reflect actual inline `useQuery` pattern in `feature-flags.tsx`
- [ ] `architecture/frontend.mdx`: demo references removed — `demo.FormComponents.tsx`, `demo-store.ts`, `src/data/` section, TanStack Store section (these files no longer exist)
...
- [ ] `frontend-patterns.mdx` §1.7: note added that `feature-flags.tsx` has known migration debt — uses direct `fetch` in event handler (violates §1.7.6 anti-pattern rule) and inline `useQuery` (pre-migration state), pending migration to `useMutation` + `queryOptions` factory

## Out of Scope

- **ADR skill `meta.json` format mismatch**: The `/adr` skill expects a JSON array `{number, title, status, date, file}` but `docs/architecture/adr/meta.json` uses the Fumadocs nav format `{title, pages[]}`. This requires a skill code change, not a doc edit — tracked as a follow-up.
...
- **Root `CLAUDE.md`**: The audit confirmed no inaccuracies in root `CLAUDE.md` — excluded from all slices.
